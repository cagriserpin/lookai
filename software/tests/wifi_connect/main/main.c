...
{
    wifi_storage_credential_t items[WIFI_STORAGE_MAX_NETWORKS] = {0};
    ui_status_saved_network_t ui_items[UI_STATUS_MAX_SAVED_NETWORKS] = {0};
    size_t count = 0;
    ...
    if (err != ESP_OK) {
        ...
        ui_status_set_saved_networks(NULL, 0);
        return;
    }

    for (size_t i = 0; i < count && i < UI_STATUS_MAX_SAVED_NETWORKS; i++) {
    ...
    ui_status_set_saved_networks(ui_items, (int)count);
}

static void update_wifi_status(const char *status)
{
    ui_status_update_wifi_status(
        status,
...
void app_main(void)
{
    ...
    /*
     * Captive portal clients often open/close sockets aggressively while probing
     * for internet access. These warnings are expected in captive portal mode
     * and can slow the system down when they spam the serial console.
     */
    ...
    ui_status_callbacks_t callbacks = {
        .connect_another = on_connect_another_pressed,
        .connect_saved = on_connect_saved_network,
        .forget_saved = on_forget_saved_network,
    };

    ui_status_set_callbacks(&callbacks);
    ...
    xTaskCreate(app_event_task, "app_event", 12288, NULL, 5, &s_app_event_task_handle);

    wifi_ap_set_event_cb(on_wifi_ap_event);

    ui_status_show_settings();
    update_wifi_status("Starting Wi-Fi");

    xTaskCreate(boot_wifi_task, "boot_wifi", 8192, NULL, 5, &s_boot_wifi_task_handle);
}

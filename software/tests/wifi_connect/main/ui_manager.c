/**
 * @file ui_manager.c
 * @brief LVGL settings UI and Wi-Fi screen rendering.
 */
...
static ui_manager_saved_network_t s_saved_items[UI_MANAGER_MAX_SAVED_NETWORKS] = {0};
...
static void render_wifi(lv_obj_t *screen)
{
    ...
    if (s_portal_active) {
        create_button(content, "Close captive portal", 382, 56, 0xC9344A, portal_toggle_event_cb, NULL);
    } else {
        create_button(content, "Connect another network", 382, 56, 0x2D7EE8, portal_toggle_event_cb, NULL);
    }
}

static void render_saved_network_card(lv_obj_t *content, const ui_manager_saved_network_t *item)
{
    lv_obj_t *card = create_card(content);
...
static void render_saved(lv_obj_t *screen)
{
    ...
    if (s_portal_active) {
        ...
        create_button(content, "Add new network", 382, 56, 0x2D7EE8, portal_toggle_event_cb, NULL);
    }
}
